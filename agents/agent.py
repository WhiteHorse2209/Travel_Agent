...
TOOLS_SYSTEM_PROMPT = f"""You are a smart travel agency. Use the tools to look up information.
    ...
    If the user asks about the weather, you can use the weather_finder tool to get the current weather and 7-day forecast. You might need to look up the latitude and longitude of the destination first using a general search if you don't know it.
    """

TOOLS = [flights_finder, hotels_finder, weather_finder]
...
class Agent:
    ...
    def __init__(self):
        ...
        self._tools_llm = ChatGroq(model='llama-3.3-70b-versatile').bind_tools(TOOLS)
    ...
    def email_sender(self, state: AgentState):
        print('Sending email')
        email_llm = ChatGroq(model='llama-3.3-70b-versatile', temperature=0.1)  # Instantiate another LLM
